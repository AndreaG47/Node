...
#define IP_FOG_G "192.168.2.1"
#define IP_FOG_N "192.168.1.1"
#define IP_FOG_M "192.168.3.2"
...
int CreateFogDatabasePacket(char *buf, int* numbytes){
	...
	a+=2;
	DEBUG_PRINTF("CreateFogDatabasePacket: List ");
	...
	for(i=0;i<Database.size();i++)
	{
		...
		DEBUG_PRINTF2(" %s",Database[i].name);
...
int SubscriberInformationRequestAction(int* socketfd, char *temp){
	...
	CreateFogDatabasePacket(temp,&a);
	 ...
	 DEBUG_PRINTF4("SubscriberInformationRequestAction: temp: %d bytes to socket %d:%s\n",a,*socketfd,temp);
...
int CreateNewConnectionInformationPacket(unsigned int* i, char buf[], char ip[], int* counter,char dest[]){
...
//	*counter+=1;
	...
	*counter+=sizeof(uint32_t);
	...
	//memcpy(buf+*counter,Database[(*i)].name,1);
	memcpy(buf+*counter,dest,1);
	*counter+=1;
	...
	for (;p<*numbytes;p++){
		...
		for (i=0;i<Database.size();i++){
			if((Database[i].name[0]==pbuf[p])){
				...
				CreateNewConnectionInformationPacket(&i,temp,ipstr,&m,destname);

				if (send(Database[i].socket,temp,m,0)==-1){ //TODO:IMPORTANT: Change *sockfd back to send(Database[i].socket
...
int ReceivedNewConnectionInfo(char buf[],vector<struct FogDatabase>* list, int* i){
	...
	info.socket=*i;
	DEBUG_PRINTF2("ReceivedNewConnectionInfo: socket: %d\n",info.socket);

	list->push_back(info);
	DEBUG_PRINTF("ReceivedNewConnectionInfo: Info added to database\n");
	DEBUG_PRINTF2("ReceivedNewConnectionInfo: %d elements in database\n",list->size());
...
//						break;
					case 'D': //Receive information request from FOG
						...
						break;
					case 'E':
						ReceivedNewConnectionInfo(buf,&Database,&(fd_list[i]));
