...
int ForwardPacket(char buf[], int *fd, int *size, int src)
{
	...
	DEBUG_PRINTF3("ForwardPacket: time = %u.%06u\n",now.tv_sec, now.tv_usec);
...
int CreateNewFogConnection(char buf[], vector<int> *FogFds, fd_set *fdset){
...
int main(void)
{
	...
	vector<int> fd_list, fdfog_list, fdfogclient_list;
	fd_list.clear();
	fd_set fds, fds_back, fdscloud, fdscloud_back, fdsfog, fdsfog_back, fdsfogclient, fdsfogclient_back;
	...
	fdfogclient_list.push_back(0);

	char buf[MAXDATASIZE];
	int numbytes, max_fd, max_fdfog, max_fdfogclient,i;
	...
	max_fdfogclient=*max_element(fdfogclient_list.begin(),fdfogclient_list.end());

	DEBUG_PRINTF2("Main: Max_fd= %d\n",max_fd);
	DEBUG_PRINTF2("Main: Max_fdfog= %d\n",max_fdfog);
	DEBUG_PRINTF2("Main: Max_fdfogclient= %d\n",max_fdfogclient);
	...
	while(1) {  // main accept() loop
		...
		FD_ZERO(&fdsfog);
		FD_ZERO(&fdscloud);
		FD_ZERO(&fdsfogclient);

		fds=fds_back;
		fdsfog=fdsfog_back;
		fdscloud=fdscloud_back;
		fdsfogclient=fdsfogclient_back;
		...
		select(max_fdfogclient+1,&fdsfogclient,NULL,NULL,&tv);
...
//					break;
				...
				case 'E':
					...
					case 'A': //Receive from cloud command to connect to other fog
						...
						CreateNewFogConnection(buf,&fdfogclient_list, &fdsfogclient_back);
		...
		for (i=1; i<fdfog_list.size();i++) { //Fog existing incoming connections
			if (FD_ISSET(fdfog_list[i],&fdsfog)){
				DEBUG_PRINTF2("Fog existing connections: socket %d \n",fdfog_list[i]);

				memset(buf,0,sizeof(buf));
				numbytes = recv(fdfog_list[i], buf, MAXPACKETSIZE-1, 0);
		...
		for (i=1;i<fdfogclient_list.size();i++){ //Fog existing outgoing connections
			if (FD_ISSET(fdfogclient_list[i],&fdsfogclient)){

				memset(buf,0,sizeof(buf));
				numbytes = recv(fdfogclient_list[i], buf, MAXDATASIZE-1, 0);

				if (numbytes == -1) {
					    perror("Main:Fog outgoing connection: recv");
					    //exit(1); //TODO: Remove this exits because it's causing that the server disconnects
					}
				if (numbytes==0){
					FD_CLR(fdfogclient_list[i],&fdsfogclient_back);
					close(fdfogclient_list[i]);
					fd_list.erase(fdfogclient_list.begin()+i);

				}
				else {
					buf[numbytes] = '\0';
					DEBUG_PRINTF3("Main: fog outgoing connections: Received %d bytes: '%s'\n",numbytes,buf);
		...
		if (ConnectionRequest==false){
			...
			if (sendtofog && (fdfogclient_list.size()>1)){
				for (i=1;i<fdfogclient_list.size();i++){
					PacketGeneration(&(fdfogclient_list[i]),&TimeA,&TimeB,&TimeC);
